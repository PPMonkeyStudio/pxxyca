function saveNews() {
		...
		if (xhr.readyState == 4) {
			if (xhr.status == 200) {
				...
				if (xhr.responseText == "premission_none") {
					...
					window.location = "/pxxyca/snews/news_page_list_news?page=page_list_news&page_list_news.pageIndex=1";
				}
	...
	if (span_option.innerHTML == 'update') {
		xhr.open("POST", "/pxxyca/snews/news_updateNews");
	} else {
		xhr.open("POST", "/pxxyca/snews/news_saveNews");
	}
	xhr.send(formData);
	start_load();
}