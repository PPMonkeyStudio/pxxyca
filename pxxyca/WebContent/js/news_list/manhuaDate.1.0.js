/*******************************************************************************
 * 漫画Jquery时间插件 编写时间：2012年7月14号 version:manhuaDate.1.0.js
 ******************************************************************************/
$(function() {
	$.fn.manhuaDate = function(options) {
		...
		$("<option value='2017'>" + 2017 + "年</option>").appendTo($("#year"));

		$("<option value='9'>" + 9 + "月</option>").appendTo($("#month"));
		...
		function getMonthNum(month, year) {
			month = month - 1;
			var LeapYear = ((year % 4 == 0 && year % 100 != 0) || year % 400 == 0) ? true
					: false;
			var monthNum;
		...
		$("#calender td").off().on(
				...
				function() {
					...
					if (dv != "&nbsp;") {
						...
						if (options.isTime) {
							var nd = new Date();
							str = $("#year").val() + options.fuhao
									+ $("#month").val() + options.fuhao + dv
									+ " " + nd.getHours() + ":"
									+ nd.getMinutes() + ":" + nd.getSeconds();
						} else {
							str = $("#year").val() + options.fuhao
									+ $("#month").val() + options.fuhao + dv;
						}
